...
// OMEGALIB HACK START
    // Do not show window by default, omegalib will take care of this.
...
void WGLWindow::show() 
{
	ShowWindow(_wglWindow, SW_SHOW);
    SetWindowPos(_wglWindow, HWND_TOP, 0,0,0,0, SWP_NOMOVE | SWP_NOSIZE);
}

void WGLWindow::hide() 
{
	ShowWindow(_wglWindow, SW_HIDE);
}

void WGLWindow::move(int x, int y, int w, int h)
{
    // window
    DWORD windowStyleEx = WS_EX_APPWINDOW;
    DWORD windowStyle = WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_OVERLAPPEDWINDOW;
    ...
    SetWindowPos(_wglWindow, HWND_TOP, 
        rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top,
        SWP_NOZORDER);
}

void WGLWindow::bringToFront()
{
    SetWindowPos(_wglWindow, HWND_TOP, 
        0, 0, 0, 0,
